...
function normCell(cell) {
  ...
  if (
    'object' == typeof cell &&
    ('v' in cell || 'w' in cell || 'l' in cell || 't' in cell)
  ) {
    ...
    url =
      ...
      (text && (text.startsWith('http') || text.includes('://')) ? text : '');
    return {
      text: String(null == text ? '' : text).trim(),
      url: url,
      isHyperlink: !!cell.l,
    };
  }
  ...
  return {
    text: text,
    url: text && (text.startsWith('http') || text.includes('://')) ? text : '',
    isHyperlink: false,
  };
}
...
function splitMulti(cell) {
  ...
  let text =
    'object' == typeof cell ? (void 0 !== cell.w ? cell.w : cell.v) : cell;
    ...
    ? text
        ...
        .map((item) => {
          ...
          return {
            text: textStr,
            url:
              textStr && (textStr.startsWith('http') || textStr.includes('://'))
                ? textStr
                : '',
            isHyperlink: false,
          };
...
const pdfHref = (pdf) =>
  pdf && pdf.url && pdf.url.toString().trim() ? pdf.url : '#';
            ...
            for (let row = merge.s.r; row <= merge.e.r; ++row) {
              for (let col = merge.s.c; col <= merge.e.c; ++col) {
                ...
                if (!worksheet[cell]) {
                  worksheet[cell] = {
                    t: 's',
                    v: worksheet[startCell] ? worksheet[startCell].v : '',
                  };
                }
              }
            }
...
function processSheet(worksheet, mergedCells = {}, manualGlossaryLink = '') {
  ...
  const maxRow = Math.max(
    ...keys.map((cell) => XLSX.utils.decode_cell(cell).r)
  );
  const rows = [];
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    const moduleCell =
      moduleCol >= 0 && row[moduleCol] ? normCell(row[moduleCol]).text : '';
    const pdfCell =
      pdfIndex >= 0
        ? XLSX.utils.encode_cell({ r: i + headerIndex + 1, c: pdfIndex })
        : '';
    const linkCell =
      linkIndex >= 0
        ? XLSX.utils.encode_cell({ r: i + headerIndex + 1, c: linkIndex })
        : '';

    const isPdfMerged =
      !pdfCell || !mergedCells[pdfCell] || mergedCells[pdfCell] === pdfCell;
    const isLinkMerged =
      !linkCell || !mergedCells[linkCell] || mergedCells[linkCell] === linkCell;

    const pdfs =
      isPdfMerged && pdfIndex >= 0 && row[pdfIndex]
        ? splitMulti(row[pdfIndex])
        : [];
    const links =
      isLinkMerged && linkIndex >= 0 && row[linkIndex]
        ? splitMulti(row[linkIndex])
        : [];

    let titleCell =
      titleIndex >= 0 && row[titleIndex] ? normCell(row[titleIndex]).text : '';
    ...
    if (!moduleCell && titleCell && /^Module\s*\d+:/.test(titleCell)) {
      ...
      if (moduleMatch) {
        currentModule =
          moduleMatch[1] + (moduleMatch[2] ? ': ' + moduleMatch[2].trim() : '');
        continue;
      }
    }

    if (
      (moduleCell && moduleCell.toLowerCase().includes('glossary')) ||
      (titleCell && titleCell.toLowerCase().includes('glossary'))
    ) {
  ...
  const filteredResources = resources.filter(
    (resource) =>
      (resource.pdfs && resource.pdfs.length) ||
      (resource.links && resource.links.length)
  );
...
function generateResourceHTML(glossaryLink, resources) {
  ...
  const generateResourceList = (resource) => {
    ...
    return (
      list +
      '  </ul><hr class="bg-yellow-400 border-0 mx-3 my-2 opacity-75 pt-1 shadow">\n'
    );
  };
  ...
  for (const resource of resources) {
    if (resource.module) {
      hasModules = true;
      if (!groupedResources[resource.module])
        groupedResources[resource.module] = [];

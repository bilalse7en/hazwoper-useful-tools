...
function extractOverview(elementsArray) {
  ...
  if (overviewStart !== -1) {
      ...
      (el, i) =>
        ...
        (el.textContent.trim().toLowerCase().includes('course objectives') ||
          ...
          el.textContent
            .trim()
            .toLowerCase()
            .includes('course facts heading 1') ||
    ...
    const finalOverviewEnd =
      overviewEnd !== -1
        ? overviewEnd
        : syllabusStart !== -1
          ? syllabusStart
          : elementsArray.length;
    const overviewElements = elementsArray.slice(
      overviewStart + 1,
      finalOverviewEnd
    );
    courseData.overview = overviewElements.map((el) => el.outerHTML).join('');
    courseData.overviewSections = [
      { heading: 'Overview', content: courseData.overview },
    ];
  }
}
...
function extractCourseObjectives(elementsArray) {
  ...
  const objectivesElements = elementsArray.slice(
    objectivesStart,
    objectivesEnd
  );
...
function extractAllSyllabusContent(elementsArray) {
  ...
  for (let i = 0; i < elementsArray.length; i++) {
    ...
    const text = element.textContent.trim().toLowerCase();
    if (
      (text.includes('course objectives') || text.includes('1.2')) &&
      (element.tagName.match(/^H[1-6]$/i) ||
        text.match(/^\d+\.\d*\s*.*course\s*objectives/i))
    ) {
      for (let j = i + 1; j < elementsArray.length; j++) {
        const nextText = elementsArray[j].textContent.trim().toLowerCase();
        if (
          nextText.includes('syllabus') ||
          nextText.includes('1.3') ||
  ...
  for (
    let i = objectivesEndIndex !== -1 ? objectivesEndIndex : 0;
  ...
  for (let i = syllabusStartIndex; i < elementsArray.length; i++) {
    ...
    if (
      lowerText.includes('course objectives') ||
  ...
  modules.forEach((module) => {
    if (module.lessons && module.lessons.length > 0) {
      module.lessons = sortNumberedEntries(module.lessons, (lesson) =>
        getLessonNumber(lesson.title)
      );
    }
  });

  return sortNumberedEntries(modules, (module) =>
    getModuleNumber(module.title)
  );
}
...
function extractLessonsDirectly(elementsArray, startIndex) {
  ...
  if (defaultModule.lessons.length > 0) {
    defaultModule.lessons = sortNumberedEntries(
      defaultModule.lessons,
      (lesson) => getLessonNumber(lesson.title)
    );
...
function extractFAQContent(elementsArray) {
  ...
  elementsArray.forEach((element, i) => {
    const text = element.textContent.trim().toLowerCase();
    if (
      (text.includes('faq') || text.includes('frequently asked questions')) &&
      element.tagName.match(/^H[1-6]$/i) &&
      faqStart === -1
    ) {
      faqStart = i;
  ...
  for (let i = faqStart + 1; i < elementsArray.length; i++) {
    ...
    const text = element.textContent.trim();
    if (
      text.includes('For Online Course Page:') ||
      (element.tagName.match(/^H[1-6]$/i) &&
...
export function generateCourseObjectivesCode(data) {
  ...
  if (data.courseObjectivesList.length > 0) {
    listItemsHtml = data.courseObjectivesList
      .map((item) => `<li>${item}</li>`)
      .join('\n');
  } else {
...
export function generateBlogCode(blogData, featuredImage = {}, imageUrls = []) {
  ...
  if (featuredImage.url) {
    ...
    if (featuredImage.title)
      imgTag += ` title="${escapeAttr(featuredImage.title)}"`;
...
<style>
	...
	.fancy-line::after { content: "Γ£ª Γ£ª Γ£ª"; position: absolute; top: -12px; left: 50%; transform: translateX(-50%); background: white; padding: 0 10px; color: red; }
...
export function extractBlogFAQContent(html) {
  ...
  for (let i = faqStart + 1; i < elementsArray.length; i++) {
    ...
    if (
      lowerText.includes('for online course page:') ||

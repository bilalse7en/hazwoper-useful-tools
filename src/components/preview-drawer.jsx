...
import {Eye, Copy, X, Check, Code as CodeIcon, Layout, Sparkles} from "lucide-react";
...
export function PreviewDrawer({
	...
	return (
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent side="right" className="!w-[90vw] p-0 border-l shadow-2xl flex flex-col h-full bg-background overflow-hidden" data-hide-default-close="true">
				<SheetHeader className="flex flex-row items-center justify-between border-b border-border p-4 bg-muted/10 backdrop-blur-md sticky top-0 z-10">
					<div className="flex items-center gap-4">
						<div className="flex items-center gap-2">
							...
							<SheetTitle className="text-xl font-black tracking-tight">{title} <span className="text-primary">Editor</span></SheetTitle>
						</div>
							...
							<div className="space-y-3">
								<div className="bg-gradient-to-br from-primary/10 via-background to-primary/5 border border-primary/20 p-4 rounded-2xl mb-4 shadow-sm relative overflow-hidden group">
									...
									<p className="text-[11px] text-muted-foreground leading-snug max-w-2xl">
										Refine and copy FAQs. <span className="text-green-600 font-bold">Green</span> indicates copied to clipboard.
									</p>
								</div>
								...
								<div className="space-y-3">
									{data.map((faq, idx) => {
										...
										return (
											<div key={idx} className={`group border rounded-2xl overflow-hidden transition-all duration-500 p-4 relative ${
												(isQCopied || isACopied) 
												...
												<div className="flex justify-between items-start gap-3 mb-3">
													<div className="flex items-start gap-3 flex-1">
														<span className={`flex items-center justify-center w-7 h-7 rounded-lg font-bold text-xs shrink-0 shadow-sm ${
															isQCopied ? 'bg-green-500 text-white' : 'bg-primary/10 text-primary'
														}`}>
															{idx + 1}
														</span>
														<div className="flex flex-col gap-0.5 flex-1">
															<span className="text-[9px] font-bold text-muted-foreground/60 uppercase tracking-widest">Question</span>
															<div 
																contentEditable
													...
													>
														...
														<span className="text-[10px] font-bold uppercase">{isQCopied ? "Copied" : "Copy"}</span>
													</Button>
												</div>
												
												{/* Condensed Divider */}
												<div className={`h-px transition-all duration-700 ${
													isQCopied || isACopied ? 'bg-green-100' : 'bg-border/30'
												} my-2`} />

												{/* Answer Body */}
												<div className="flex justify-between items-start gap-3">
													<div className="flex-1 pl-10">
														<span className="text-[9px] font-bold text-muted-foreground/60 uppercase tracking-widest mb-1 block">Answer</span>
														<div 
															contentEditable
															suppressContentEditableWarning
															className={`prose prose-xs dark:prose-invert max-w-none text-muted-foreground text-xs leading-normal outline-none focus:ring-1 focus:ring-primary/20 rounded-lg p-3 transition-all duration-500 ${
																isACopied ? 'bg-green-500/5 text-green-900 dark:text-green-100' : 'bg-muted/20'
															}`}
															onBlur={(e) => {
													...
													>
														...
														<span className="text-[10px] font-bold uppercase">{isACopied ? "Copied" : "Copy"}</span>
													</Button>
												</div>
											</div>
										);

      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
        <div className="space-y-4">
          ...
          <p className="text-muted-foreground font-medium max-w-xl leading-relaxed">
            Expert insights on HAZWOPER automation, safety technology, and digital compliance strategies.
          </p>
        </div>
        
        <Button variant="outline" className="h-12 rounded-xl px-6 border-border hover:bg-primary/5 hover:text-primary transition-all font-bold gap-2 group" asChild>
          <Link href="/blog">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {loading ? (
          Array(6).fill(0).map((_, i) => (
            <div key={i} className="h-[400px] rounded-[32px] bg-muted animate-pulse" />
          ))
        ) : (

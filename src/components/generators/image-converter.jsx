...
export function ImageConverter() {
  ...
  const downloadSingle = (file) => {
    performAction(() => downloadImage(file.blob, file.fileName), {
      type: 'download',
      name: file.fileName,
    });
  };
  ...
  const totalOriginalSize = convertedFiles.reduce(
    (sum, f) => sum + (f.originalSize || 0),
    0
  );
  const totalConvertedSize = convertedFiles.reduce(
            ...
            <div className="rounded-[2rem] bg-card/30 border border-border/50 backdrop-blur-md shadow-[inset_0_1px_1px_rgba(var(--glass-shadow-highlight),0.08),inset_0_10px_30px_rgba(var(--glass-shadow-color),0.08)] p-6 animate-in zoom-in duration-500">
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-green-500/5 border border-green-500/20 mb-4">
                <div className="flex flex-col">
                  <span className="text-[8px] font-black text-green-500 uppercase tracking-widest mb-1">
                    Total Savings
                  </span>
          ...
          <div className="flex-1 flex flex-col gap-5 p-6 rounded-[2.5rem] bg-card/40 border border-border/50 backdrop-blur-xl relative overflow-hidden shadow-[inset_0_1px_1px_rgba(var(--glass-shadow-highlight),0.08),inset_0_20px_50px_rgba(var(--glass-shadow-color),0.08)]">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <span className="text-[8px] font-black uppercase tracking-wider text-primary pl-1">
                  Convert To
                </span>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant="outline"
                  className="border-primary/30 text-primary text-[9px] font-black"
                >
                  {quality}%
                </Badge>
              </div>
              ...
              <div className="space-y-4 p-4 rounded-xl bg-card/30 border border-border/50 animate-in slide-in-from-top-2 duration-300 shadow-[inset_0_2px_8px_rgba(var(--glass-shadow-color),0.06)]">
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex items-center justify-between p-3 rounded-xl border border-border/50 bg-background/30">
                    <label className="text-[8px] font-black uppercase tracking-wider text-muted-foreground">
                      Aspect Lock
                    </label>
                    <Switch
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[8px] font-black uppercase tracking-wider text-muted-foreground pl-1">
                      Width
                    </label>
                    <input
                      type="number"
                      ...
                      className="w-full h-10 px-3 rounded-lg border border-border/50 bg-background/50 text-sm font-mono font-bold placeholder:text-muted-foreground/50 outline-none focus:border-primary/50 transition-colors"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[8px] font-black uppercase tracking-wider text-muted-foreground pl-1">
                    ...
                    <input
                      ...
                      className="w-full h-10 px-3 rounded-lg border border-border/50 bg-background/50 text-sm font-mono font-bold placeholder:text-muted-foreground/50 outline-none focus:border-primary/50 transition-colors disabled:opacity-50"
                    />
                  </div>
                </div>
              </div>
            ...
            {!isConverting && (
              <div className="flex flex-col items-center justify-center -space-y-0.5">
                ...
                <span className="text-[10px] font-medium opacity-70">
                  ...
                  {toFormat.toUpperCase() || 'Select format'}
                </span>
              </div>
            )}
          </ProgressButton>

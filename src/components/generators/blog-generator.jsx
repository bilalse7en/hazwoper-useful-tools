  ...
  const handleGenerateBlog = () => {
      ...
      if (
        ...
        while (current.nextElementSibling) {
          ...
          if (
            nextTag &&
            nextTag.match(/^h[1-6]$/) &&
            !next.textContent.toUpperCase().includes('FAQ')
          ) {
            break;
          }
    ...
    if (featuredImage) {
      ...
      if (h1) {
        ...
        if (
          !existingImg ||
          existingImg.tagName !== 'P' ||
          !existingImg.querySelector('img')
        ) {
    ...
    tempDiv.querySelectorAll('table').forEach((table) => {
      ...
      if (thead) {
        thead
          .querySelectorAll('tr')
          .forEach((tr) => tr.classList.add('bg-warning'));
      } else {
        ...
        const allBold =
          ...
          firstRowCells.every(
            (cell) => cell.querySelector('b, strong') || cell.tagName === 'TH'
          );
        if (hasTh || allBold) {
      ...
      rows.forEach((tr) => {
        ...
        if (cells.length > 0) {
          ...
          const isLeftHeading =
            firstCell.tagName === 'TH' || firstCell.querySelector('b, strong');
          if (isLeftHeading) {
...
<style>
  ...
  const handleGenerateFAQ = () => {
    ...
    if (!blogData.faqData || blogData.faqData.length === 0) {
      return showNotification(
        "No FAQs found in the document. Make sure your document has a section with 'FAQ' or 'FAQs' heading.",
        'warning'
      );
    }
    ...
    setActiveView('faq');
    showNotification(
      `FAQ code generated! ${blogData.faqData.length} FAQ${blogData.faqData.length > 1 ? 's' : ''} found.`,
      'success'
    );
  ...
  const confirmReset = () => {
    ...
    showNotification(
      'Everything has been reset! Ready for a fresh start.',
      'success'
    );
  };
  ...
  const handleAnalyzeFile = async () => {
    if (!analyzerFile)
      return showNotification('Please select a file to analyze.', 'warning');
    ...
    try {
      ...
      for (let i = 0; i < rawBlocks.length; i++) {
        ...
        const isMetadata = unwantedMetadata.some((pattern) => {
          ...
          return (
            ...
            (lowerContent.includes(p) && lowerContent.length < p.length + 15)
          );
        });
        ...
        if (
          !encounteredFAQ &&
          ((isHeading && hasFAQWords) ||
            (hasFAQWords && lowerContent.length < 50))
        ) {
          encounteredFAQ = true;
        ...
        if (!encounteredFAQ && isMetadata) {
          ...
          if (i + 1 < rawBlocks.length) {
            ...
            if (
              !['H1', 'H2', 'H3'].includes(nextBlock.type) &&
              !unwantedMetadata.some((p) => nextLower.includes(p))
            ) {
              i++;
            }
          }
          ...
          if (questionMatch) {
            ...
            if (
              ...
              if (!answerText.toLowerCase().startsWith('answer:')) {
                ...
                const valAfterColon = answerText
                  .substring(answerText.indexOf(':') + 1)
                  .trim();
      ...
      const htmlForFAQ = (data.rawStructure || [])
        .map((block) => {
      ...
      if (faqData && faqData.length > 0) {
        ...
        setFaqCode(faqCode);
        showNotification(
          'File analyzed! FAQ section detected and generated automatically.',
          'success'
        );
      } else {
        showNotification(
          'File analyzed! Now starting Step-by-Step review.',
          'success'
        );
      }
  ...
  const handleFinishReview = async () => {
    ...
    try {
      ...
      const elements = tempDiv.querySelectorAll(
        'h1, h2, h3, p, ul, ol, table, .table-container'
      );
      ...
      elements.forEach((el) => {
        ...
        if (['H1', 'H2', 'H3', 'P'].includes(tagName)) {
          ...
          const tableHtml =
            tagName === 'TABLE'
              ? el.outerHTML
              : el.querySelector('table')?.outerHTML || el.innerHTML;
          updatedStructure.push({
            type: 'TABLE',
            content: tableHtml,
          });
      ...
      formData.append(
        'structure',
        JSON.stringify(
          updatedStructure.length > 0 ? updatedStructure : rawStructure
        )
      );
      ...
      try {
        ...
        if (blogData.faqData && blogData.faqData.length > 0) {
          ...
          setFaqCode(faqCode);
          showNotification(
            `Perfect file saved! ${blogData.faqData.length} FAQs detected and ready.`,
            'success'
          );
        } else {
        ...
        showNotification(
          'File saved but processing failed: ' + error.message,
          'warning'
        );
      }
    } catch (error) {
    ...
    <Fragment>
              ...
              <CardContent className="card-body space-y-4">
                {!showReview ? (
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <p className="text-xs text-muted-foreground">
                          Click to upload Word file for analysis
                        </p>
                      </div>
                      <Input
                  ...
                  <div className="flex flex-col items-center justify-center p-8 text-center bg-purple-500/5 rounded-xl border-2 border-dashed border-purple-200 animate-pulse">
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      Finish the perfection window to continue.
                    </p>
                  </div>
                )}
                ...
                {fixedFileUrl && !showReview && (
                  <div className="space-y-3 pt-4 border-t border-green-200 dark:border-green-800">
                    ...
                    >
                      <Download className="mr-2 h-4 w-4" /> Download Perfect
                      Word File
                    </Button>
                ...
                {blogData && blogData.imageCount > 0 && (
                  <div className="space-y-2 border-t pt-4">
                    ...
                    <ScrollArea className="h-[150px] pr-4">
                      <div className="space-y-2">
                        {Array.from({ length: blogData.imageCount }).map(
                          (_, i) => (
                  ...
                  <div className="flex items-center justify-between mb-2 mt-2">
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        onClick={() => {
                          const content =
                            activeView === 'content' ? blogCode : faqCode;
                          const title =
                            activeView === 'content' ? 'Blog Post' : 'FAQ';
      ...
      {showReview && (
        <div className="fixed inset-0 z-[100] bg-background flex flex-col animate-in fade-in duration-200">
          {/* Compact Header */}
          <div
            className="p-2 px-4 border-b flex items-center justify-between bg-card dark:bg-slate-900 shadow-sm"

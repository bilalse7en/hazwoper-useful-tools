...
if (
  ...
  ffmpegPath =
    possiblePaths.find((p) => isValidFFmpegPath(p)) || possiblePaths[0];
}
    ...
    async start(controller) {
      ...
      const sendProgress = (percent, message) => {
        ...
        try {
          controller.enqueue(
            encoder.encode(
              `data: ${JSON.stringify({ type: 'progress', percent, message })}\n\n`
            )
          );
        } catch (e) {
          isClosed = true;
      ...
      const sendComplete = (buffer) => {
        ...
        try {
          ...
          controller.enqueue(
            encoder.encode(
              `data: ${JSON.stringify({ type: 'complete', data: base64, size: buffer.length })}\n\n`
            )
          );
          controller.close();
        } catch (e) {}
      ...
      try {
        ...
        if (!isValidFFmpegPath(ffmpegPath)) {
          sendError(
            `FFmpeg binary not found at: ${ffmpegPath}. Please ensure ffmpeg-static is installed correctly by running: npm install ffmpeg-static`
          );
          return;
        }
        ...
        if (targetSizeMB && duration && duration > 0) {
          const targetVideoBitrate = calculateTargetBitrate(
            targetSizeMB,
            duration,
            audioBitrateKbps
          );
        ...
        await new Promise((resolve, reject) => {
          ...
          const timeout = setTimeout(() => {
            ...
            reject(
              new Error(
                'Video compression timed out after 1 hour. Please try with a shorter video or different settings.'
              )
            );
          }, 3600000); // 1 hour

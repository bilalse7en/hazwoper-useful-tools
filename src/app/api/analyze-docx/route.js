...
function htmlTableToDocxTable(html) {
  ...
  while ((trMatch = trRegex.exec(html)) !== null) {
    ...
    while ((tdMatch = tdRegex.exec(rowContent)) !== null) {
      ...
      if (
        cellTag.includes('text-center') ||
        cellTag.includes('text-align: center')
      ) {
        alignment = AlignmentType.CENTER;
      } else if (
        cellTag.includes('text-end') ||
        cellTag.includes('text-align: right')
      ) {
    ...
    if (item.type === 'H3' || item.type === 'P') {
      ...
      if (split.hasQA) {
        ...
        if (split.answer) {
          docChildren.push(
            new Paragraph({
              children: htmlToTextRuns(
                item.innerHtml || split.answer,
                item.isBold
              ),
            })
          );
    ...
    switch (item.type) {
      ...
      case 'TABLE':
        ...
        paragraph = new Paragraph({
          children: [
            new TextRun({
              text: '[Table contents converted to text]',
              italic: true,
            }),
          ],
        });
    ...
    styles: {
      ...
      paragraphStyles: [
        ...
        },
        {
          ...
          paragraph: { spacing: { after: 150, before: 300 } },
        },
        {
          id: 'Heading3',
          name: 'Heading 3',
          ...
          paragraph: { spacing: { after: 100, before: 200 } },
        },
      ],
    },
    sections: [{ children }],
...
function analyzeDocumentStructure(html) {
  ...
  while ((match = paragraphRegex.exec(html)) !== null) {
    ...
    if (tagName === 'LI') {
      ...
      const segment = html.substring(
        Math.max(0, match.index - 50),
        match.index
      );
      if (
        segment.toLowerCase().lastIndexOf('<ol') >
        segment.toLowerCase().lastIndexOf('<ul')
      ) {
        listType = 'OL';
      } else {
        listType = 'UL';
    ...
    subLines.forEach((line) => {
      ...
      if (
        plainText.match(
          /^(Meta Description|Meta Title|Slug|Category|Relevant Courses|Training):/i
        )
      )
        return;
      if (h1Found && plainText.match(/^(https?:\/\/|www\.|envato|adobestock)/i))
        return;
  ...
  matches.forEach((item, index) => {
    ...
    // Rule 3: Detect FAQ Section early
      ...
      text.match(/^(FAQs?|Frequently Asked Questions|1\.5\.|Question)/i)
    ) {
      isFAQSection = true;
...
function validateSEO(structure) {
  ...
  if (h1Count === 0) {
    ...
    warnings.push(
      `Multiple H1 headings found (${h1Count}). First one kept, others converted to H2.`
    );
  }
  ...
  if (h2Count === 0) {
    warnings.push(
      'No H2 headings found. Consider breaking content into main sections.'
    );
  } else if (h2Count < 2) {
    warnings.push(
      'Few H2 headings. Consider adding more main sections for better SEO.'
    );
  }

...
import { ArrowLeft, Save, Globe, FileText, Layout, User, Clock, AlertCircle, RefreshCw, Sparkles, Upload, Image as ImageIcon } from "lucide-react";
...
export default function AdminBlogEditPage({ params }) {
    ...
    try {
      ...
      if (error) {
        if (error.message.includes("Bucket not found")) {
          throw new Error("Neural Storage Error: Please create a 'media' bucket in Supabase Storage with public access.");
        }
        throw error;
      }
      ...
      console.error("Upload Error Details:", err.message, err.details, err.hint);
    } finally {
      setSaving(false);
    }
  };
  ...
  const executeAIGeneration = async () => {
    ...
    setGenerating(true);
    toast.loading(`Synthesizing ${aiInput.wordCount} word technical sequence...`, { id: 'ai-gen' });
    ...
    try {
      ...
      const appTools = [
        { name: "Image Conversion Engine", desc: "Our high-fidelity converter supports WEBP, PNG, and JPG while maintaining industrial metadata standards." },
        { name: "HTML Structural Deep-Cleaner", desc: "A precision tool for sanitizing messy regulatory code into pristine, accessible HTML5." },
        { name: "HAZWOPER Productivity Suite", desc: "The foundational ecosystem designed for environmental safety professionals and architectural compliance." },
        { name: "Document Neural Analyzer", desc: "Automated extraction and analysis of complex technical documentation for HAZWOPER reporting." }
      ];
      ...
      const extendedContent = Array.from({ length: extendedBodyCount }).map((_, i) => `
      ...
      toast.success("Long-form Professional Sequence Synchronized.", { id: 'ai-gen' });
    } catch (err) {
  ...
  const generateAIImage = async () => {
    ...
    toast.loading("Neural artist is drawing and persisting your hero image...", { id: 'img-gen' });
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to capture neural stream via proxy.");
      
      const rawBlob = await response.blob();
      const rawFile = new File([rawBlob], "raw_gen.jpg", { type: 'image/jpeg' });

      // 3. Neural Optimization (Compression & Conversion)
      const optimized = await convertImage(rawFile, 'webp', { quality: 80, width: 1280 });
      ...
      toast.success(`AI Asset Optimized (${optimized.reduction}% reduction) & Persisted.`, { id: 'img-gen' });
    } catch (err) {
      toast.error(err.message || "Neural drawing or persistence failed.");
      console.error("Neural Error Details:", err.message, err.details, err.hint);
    } finally {
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
        ...
        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground animate-pulse">Decrypting Sequence...</span>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto px-6 py-12 space-y-12">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
             <h1 className="text-4xl md:text-5xl font-black tracking-tight tracking-tighter">
               {isNew ? 'Initialize' : 'Refine'} <span className="text-primary">Sequence</span>
             </h1>
             <p className="text-muted-foreground font-medium text-sm">
               {isNew ? 'Configure a new professional insight for the global editorial archive.' : `Updating registry entry for sequence ID: ${id.substring(0, 8)}`}
             </p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-8">
           <Card className="rounded-[40px] shadow-2xl border-border bg-card/40 backdrop-blur-xl p-10 space-y-10">
              <div className="space-y-4">
                 ...
                 <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Editorial Body (HTML Support)</label>
                      <textarea 
                        name="content" 
           ...
           <Card className="rounded-[40px] shadow-2xl border-border bg-card/40 backdrop-blur-xl p-10 space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Archive Description (for Cards)</label>
                <textarea 
                   name="description" 
        ...
        <div className="space-y-8">
           ...
           <Card className="rounded-[40px] shadow-2xl border-border bg-card/40 backdrop-blur-xl p-8 space-y-6">
                ...
                {imageGenerating ? (
                  ...
                  <>
                    ...
                    <div className="text-center">
                       <p className="text-[10px] font-black uppercase tracking-widest">Upload Resource</p>
                       <p className="text-[9px] text-muted-foreground font-medium">PNG, JPG up to 10MB</p>
                    </div>
                  </>
                )}
                <input 
              ...
              >
                <Sparkles className={cn("w-3.5 h-3.5", imageGenerating && "animate-pulse")} />
                {imageGenerating ? 'Drawing...' : 'Neural Image Spark'}
              </Button>
              <div className="space-y-2">
                 <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Or Paste Asset URL</label>
                 <Input 
                   name="image_url" 
           ...
           <Card className="rounded-[40px] shadow-2xl border-border bg-card/40 backdrop-blur-xl p-8 space-y-8">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Slug (URL Path)</label>
                  <Input 
                    name="slug" 
                    value={formData.slug || ""} 
                    onChange={handleChange}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Category Segment</label>
                  <Input 
                    name="category" 
                ...
                <div className="space-y-2">
                   <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground text-primary/60">Target Read Time (AI Guidance)</label>
                ...
                <div className="space-y-2">
                   <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Assigned Author (Auto)</label>
                   <div className="relative">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Publication Identity (Auto)</label>
                  <Input 
                    name="date" 
                    value={formData.date || ""} 
                    readOnly
           ...
           <div className="p-8 rounded-[32px] bg-primary/5 border border-primary/10 space-y-4">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-primary">Security Note</h4>
              <p className="text-[11px] text-muted-foreground font-medium leading-relaxed">
                Changes made here are permanently recorded in the global registry. High-impact content should be audited before final execution.
              </p>
           </div>
        </div>
      ...
      {isAIModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 animate-in-fade">
           <div className="absolute inset-0 bg-background/80 backdrop-blur-md" onClick={() => setIsAIModalOpen(false)} />
           <Card className="w-full max-w-lg relative z-10 rounded-[40px] border-border bg-card/60 backdrop-blur-2xl shadow-[0_0_100px_rgba(0,0,0,0.2)] overflow-hidden">
              ...
              <CardHeader className="p-10 pb-6">
                 ...
                 <p className="text-muted-foreground mt-2 font-medium">Define your editorial parameters to ignite the generation engine.</p>
              </CardHeader>
              <CardContent className="p-10 pt-0 space-y-8">
                 <div className="space-y-6">
                    <div className="space-y-2">
                       <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Neural Synthesis Prompt</label>
                       <Input 
                    ...
                    <div className="space-y-2">
                       <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Target Word Count</label>
                       <Input 
                          placeholder="e.g. 2000"
                          value={aiInput.wordCount}
                          onChange={(e) => setAiInput({ ...aiInput, wordCount: e.target.value })}
                          className="h-14 rounded-2xl bg-muted/30 border-border/50 text-base font-bold"
                       />
                    </div>
                 </div>

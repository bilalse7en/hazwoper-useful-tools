...
import { ArrowLeft, Save, User as UserIcon, Camera, LayoutDashboard } from "lucide-react"
...
export default function ProfilePage() {
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
        ...
        alert("Error uploading avatar. Make sure you have a 'profiles' bucket in Supabase storage.");
    } finally {
        setUpdating(false);
    }
  };
  ...
  async function saveProfile(fName, lName, full, uname, avatar) {
    ...
    if (!response.ok) {
        const error = new Error(result.message || result.error || 'Failed to update profile');
  ...
  async function handleUpdate(e) {
    ...
    try {
        const result = await saveProfile(firstName, lastName, null, username, avatarUrl);
        setFullName(result.user?.user_metadata?.full_name || `${firstName} ${lastName}`.trim());
        ...
        <Card className="shadow-2xl border-border overflow-hidden rounded-[40px] bg-card/40 backdrop-blur-xl">
          ...
          <CardHeader className="space-y-1 relative pt-0">
            <div className="flex flex-col items-center -mt-20 mb-8">
                <div className="relative group">
                    ...
                    <div className="relative">
                        <Avatar className="h-40 w-40 border-[6px] border-background shadow-2xl relative">
                            <AvatarImage src={avatarUrl || user?.avatar} className="object-cover" />
                            <AvatarFallback className="text-5xl font-black bg-muted text-primary">
                                {firstName?.charAt(0).toUpperCase() || fullName?.charAt(0).toUpperCase()}
                            </AvatarFallback>
                        </Avatar>
                ...
                <h2 className="text-4xl font-black mt-6 tracking-tight text-foreground">{fullName}</h2>
                <div className="flex items-center gap-3 mt-2">
                    <span className="text-muted-foreground text-xs uppercase tracking-[0.3em] font-bold py-1 px-3 bg-muted rounded-full">{user?.role?.replace('_', ' ')}</span>
                    {user?.has_generator_access && (
                        <Badge className="bg-primary/10 text-primary border-primary/20 hover:bg-primary/20 text-[10px] py-0.5 px-2.5 rounded-full font-black uppercase">Generator Pro</Badge>
                    )}
                </div>
            </div>
          </CardHeader>
          ...
          <form onSubmit={handleUpdate}>
            <CardContent className="space-y-8 px-10">
              <div className="grid gap-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2.5">
                      <Label htmlFor="fullName" className="text-xs font-black uppercase tracking-widest text-muted-foreground ml-1">Display Name</Label>
                      <Input 
                        id="fullName" 
                        value={fullName} 
                    ...
                    <div className="space-y-2.5">
                      <Label htmlFor="username" className="text-xs font-black uppercase tracking-widest text-muted-foreground ml-1">Unique Username</Label>
                      <Input 
                        id="username" 
                        value={username} 
                      ...
                      {suggestions.length > 0 && (
                          <div className="flex flex-wrap gap-2 mt-2 px-1 animate-in-fade">
                              <span className="text-[10px] font-bold text-amber-600 dark:text-amber-400 uppercase tracking-tighter w-full mb-1">Taken. Try one of these:</span>
                ...
                <div className="p-6 rounded-3xl bg-muted/20 border border-border/40 space-y-3 relative overflow-hidden group">
                    ...
                    <Label className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground font-black">Connected Identity</Label>
                    <div className="flex items-center justify-between">
                        <div className="text-sm font-bold tracking-tight">{user?.email}</div>
                        <Badge variant="outline" className="text-[10px] border-border/60 text-muted-foreground">Linked</Badge>
                    </div>
                </div>
              </div>
            </CardContent>
